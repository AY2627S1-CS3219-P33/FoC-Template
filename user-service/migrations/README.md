...
Roll back in reverse order. `000003_auth0_only.down.sql` restores the nullable legacy column before `000002` is rolled back. `000002_auth0_identities.down.sql` refuses to run while Auth0 accounts or duplicate usernames exist. `000001_accounts.down.sql` drops the accounts table and all account data.
